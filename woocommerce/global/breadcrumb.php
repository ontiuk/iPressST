...
if ( true !== $ip_wc_breadcrumbs ) { return; } 
...
if ( true === $ip_wc_custom_breadcrumbs ) { 
	...
	if ( is_cart() || $page_template === 'template-cart.php' ) { 

		$template = 'cart.php';

	} elseif ( is_checkout() || $page_template === 'template-checkout.php' ) {

		$template = 'checkout.php';

	} elseif ( is_account_page() || $page_template === 'template-account.php' ) {
	...
	if ( empty( $template ) ) {
		echo sprintf( 
			...
			esc_html__( 'No Woocommerce Breadcrumb for this type [%s] yet!', 'ipress' ), esc_attr( $template ) );
		return;
	}
