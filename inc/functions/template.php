...
if ( ! function_exists( 'ipress_post_date' ) ) :
	...
	function ipress_post_date( $datetime = true ) {
		...
		$ip_post_date = sprintf( '<span class="post-date">%1$s $2$s</span>', __( 'Posted on', 'ipress' ), $ip_post_time );
...
if ( ! function_exists( 'ipress_post_thumbnail' ) ) :
	...
	function ipress_post_thumbnail( $size = 'full' ) {
		...
		if ( is_singular() ) {
			...
			echo sprintf(
				'<a class="post-thumbnail" href="%s" aria-hidden="true">%s</a>',
...
if ( ! function_exists( 'ipress_loop_image' ) ) :
	...
	function ipress_loop_image( $size = 'full' ) {
		...
		if ( $image ) {
			echo sprintf(
				'<div class="post-image"><a href="%s" title="%s"><img src="%s" /></a></div>',
				esc_url( get_permalink() ),
...
if ( ! function_exists( 'ipress_post_categories' ) ) :
	...
	function ipress_post_categories() {
		...
		if ( has_category() ) {
			echo sprintf(
				'<div class="post-categories"><span>%s</span>%s</div>',
...
if ( ! function_exists( 'ipress_post_tags' ) ) :
	...
	function ipress_post_tags() {
		...
		if ( has_tag() ) {
			echo sprintf(
				'<div class="post-tags"><span>%s</span>%s</div>',
				esc_html__( 'Tagged in', 'ipress' ),
				esc_html( get_the_tag_list( '', ', ' ) )
			);
		}
	}
endif;
...
if ( ! function_exists( 'ipress_post_comments_link' ) ) :
	...
	function ipress_post_comments_link() {
		...
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo sprintf(
				'<div class="comments-link"><div class="comments-label">%s</div><span class="comments-link">%s</span></div>',
